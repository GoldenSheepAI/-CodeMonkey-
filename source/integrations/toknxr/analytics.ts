...
export class AnalyticsEngine {
	...
	generateStats(usages: TokenUsage[]): UsageStats {
		...
		for (const usage of usages) {
			...
			if (pricing) {
				const cost = (usage.inputTokens * pricing.inputPerToken) +
	...
	analyzePatterns(usages: TokenUsage[]): {
		...
		const topModels = Array.from(modelStats.entries())
			.map(([model, stats]) => ({ model, usageCount: stats.count, totalCost: stats.cost }))
		...
		const totalCost = Array.from(modelStats.values()).reduce((sum, s) => sum + s.cost, 0);
		const averageTokensPerRequest = usages.length > 0 ? totalTokens / usages.length : 0;
	...
	private generateRecommendations(
		...
		if (topModels.length > 0) {
			...
			recommendations.push(
				`Consider optimizing usage of ${highestCostModel.model} ($${highestCostModel.totalCost.toFixed(2)} total cost)`
			);
		}
		...
		if (costTrends.length >= 7) {
			...
			const avgEarlier = earlierCosts.length > 0 ?
				earlierCosts.reduce((a, b) => a + b, 0) / earlierCosts.length : avgRecent;

			const changePercent = ((avgRecent - avgEarlier) / avgEarlier) * 100;
			if (changePercent > 10) {
				recommendations.push(`Cost trend increasing by ${changePercent.toFixed(1)}% in recent days`);
			}
		}

		// Efficiency recommendations
		if (efficiency.averageTokensPerRequest < 50) {
			recommendations.push('Consider batching smaller requests to improve token efficiency');
		}

		if (efficiency.costEfficiency > 0.01) {
			recommendations.push('High cost per token detected - consider model optimization');
		}
	...
	exportData(usages: TokenUsage[], format: 'json' | 'csv' = 'json'): string {
		if (format === 'csv') {
			const headers = 'timestamp,model,inputTokens,outputTokens,totalTokens,sessionId\n';
			const rows = usages.map(usage =>
				`${usage.timestamp.toISOString()},${usage.model},${usage.inputTokens},${usage.outputTokens},${usage.totalTokens},${usage.sessionId || ''}`
			).join('\n');
			return headers + rows;
		}

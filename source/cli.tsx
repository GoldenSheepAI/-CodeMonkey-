...
const isNonInteractiveCommand =
	args.length > 0 &&
	nonInteractiveCommands.some(
		cmd => args[0] === cmd || args[0] === cmd.replace('--', ''),
	);
...
if (isNonInteractiveCommand) {
	...
	if (!process.stdin.isTTY) {
		console.error(
			'Error: CodeMonkey requires an interactive terminal (TTY) for chat mode.',
		);
		console.error('');
		console.error('Usage:');
		console.error(
			'  codemonkey                    # Interactive chat mode (requires TTY)',
		);
		...
		console.error('');
		console.error(
			'For automated scripts, use specific non-interactive commands.',
		);
		process.exit(1);
	}
